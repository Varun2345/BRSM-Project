...
A total of \textbf{185 participants} were recorded, of whom \textbf{171} had usable behavioural data files. Of these, \textbf{163} had valid demographic records; for the remaining participants, missing demographic values were \textbf{imputed} using the group mean (for age) or mode (for categorical factors), allowing all \textbf{166} analysed participants to be included in demographic balance tests (H6). Both groups were well-matched on all demographic variables; a breakdown is given in Table~2.
...
\toprule
\textbf{Variable}   & \textbf{Category} & \textbf{AB ($n=78$)} & \textbf{NB ($n=88$)} \\
\midrule
\textbf{Age}        & Range (years)     & \multicolumn{2}{c}{19--34} \\
                    & Mean (SD)         & 22.1 (1.7) & 22.2 (2.2) \\
\midrule
\textbf{Gender}     & Male              & 55 & 63 \\
                    & Female            & 23 & 23 \\
\midrule
\textbf{Handedness} & Right-handed      & 75 & 81 \\
                    & Left-handed       &  3 &  5 \\
\midrule
\textbf{Vision}     & Normal            & 45 & 46 \\
                    & Corrected         & 33 & 38 \\
                    & Uncorrected       &  0 &  2 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize $^*$ Totals for NB may sum to 86 due to two participants lacking demographic records entirely.
\end{flushleft}
\end{table}
...
\textbf{Missing demographic data.}\; Twenty-two participants who did not fill the demographic form are retained in all analyses (including H6) by imputing missing values with the group mean (age) or mode (categorical values).
