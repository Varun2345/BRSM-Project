...
def plot_h4_confidence_accuracy(trials_df):
    ...
    print("\n" + "-" * 40)
    print("H4 — Simple Effects: Confidence vs Accuracy by Condition")
    print("Testing normality (Shapiro-Wilk) and selecting appropriate test")
    print("-" * 40)
    ...
    for cond in ['AB', 'NB']:
        ...
        if len(common_ids) > 1:
            diffs = subj_corr[common_ids] - subj_incorr[common_ids]
            
            # 1. Normality Check
            shapiro_stat, shapiro_p = stats.shapiro(diffs)
            is_normal = shapiro_p > 0.05
            print(f"\n  {cond} Normality Check: W={shapiro_stat:.3f}, p={shapiro_p:.6f}")
            print(f"    Distribution is {'Normal' if is_normal else 'NOT Normal'}")
            
            # 2. Test Selection & Effect Size
            if is_normal:
                stat, p = stats.ttest_rel(subj_corr[common_ids], subj_incorr[common_ids])
                test_name = "Paired t-test"
                stat_name = "t"
                # Cohen's d for paired samples: mean_diff / std_diff
                d_paired = np.mean(diffs) / np.std(diffs, ddof=1) if np.std(diffs, ddof=1) != 0 else 0
                effect_size_str = f"Cohen's d (paired) = {d_paired:.3f}"
            else:
                stat, p = stats.wilcoxon(subj_corr[common_ids], subj_incorr[common_ids])
                test_name = "Wilcoxon Signed-Rank test"
                stat_name = "W"
                # Rank-Biserial Correlation (r)
                ranks = stats.rankdata(np.abs(diffs))
                pos_ranks = np.sum(ranks[diffs > 0])
                neg_ranks = np.sum(ranks[diffs < 0])
                total_ranks = pos_ranks + neg_ranks
                r_biserial = (pos_ranks - neg_ranks) / total_ranks if total_ranks > 0 else 0
                effect_size_str = f"Rank-Biserial r = {r_biserial:.3f}"
                
            direction = "Correct > Incorrect" if np.mean(diffs) > 0 else "Incorrect > Correct"
            print(f"    Test: {test_name}")
            print(f"    Direction: {direction}")
            print(f"    Correct M={subj_corr[common_ids].mean():.3f}, Incorrect M={subj_incorr[common_ids].mean():.3f}")
            print(f"    {stat_name}-statistic = {stat:.3f}, p = {p:.6f}")
            print(f"    {effect_size_str}")
...
def plot_h6_demographics(trials_df, demo_df):
    ...
    ax = axes[0, 0]
    for cond, color in CONDITION_COLORS.items():
        subset = demo_valid[demo_valid['condition'] == cond]['age'].dropna()
        if len(subset) > 1:
    ...
    # Age: Normality check → Independent t-test or Mann-Whitney U
    ab_age = ab_demo['age'].dropna()
    nb_age = nb_demo['age'].dropna()

    sw_ab_stat, sw_ab_p = stats.shapiro(ab_age)
    sw_nb_stat, sw_nb_p = stats.shapiro(nb_age)
    ab_normal = sw_ab_p > 0.05
    nb_normal = sw_nb_p > 0.05

    print(f"  Age Normality Check (Shapiro-Wilk):")
    print(f"    AB: W={sw_ab_stat:.3f}, p={sw_ab_p:.4f} — {'Normal' if ab_normal else 'NOT Normal'}")
    print(f"    NB: W={sw_nb_stat:.3f}, p={sw_nb_p:.4f} — {'Normal' if nb_normal else 'NOT Normal'}")

    if ab_normal and nb_normal:
        t, p_age = stats.ttest_ind(ab_age, nb_age)
        print(f"  Age: Independent t-test (both groups normal)")
        print(f"    AB M={ab_age.mean():.1f} vs NB M={nb_age.mean():.1f}, t={t:.3f}, p={p_age:.4f}")
    else:
        u_stat, p_age = stats.mannwhitneyu(ab_age, nb_age, alternative='two-sided')
        print(f"  Age: Mann-Whitney U test (normality not met)")
        print(f"    AB Mdn={ab_age.median():.1f} vs NB Mdn={nb_age.median():.1f}, U={u_stat:.1f}, p={p_age:.4f}")
