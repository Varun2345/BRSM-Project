...
def load_all_data():
    ...
    # --- DEMOGRAPHIC DATA IMPUTATION ---
    # Apply imputation: mean for age, mode for categorical fields
    if demo_df['age'].isnull().any():
        age_mean = demo_df['age'].mean()
        demo_df['age'] = demo_df['age'].fillna(age_mean)
        print(f"  Imputed missing 'age' with mean: {age_mean:.2f}")

    for col in ['gender', 'handedness', 'vision']:
        if demo_df[col].isnull().any():
            col_mode = demo_df[col].mode()[0]
            demo_df[col] = demo_df[col].fillna(col_mode)
