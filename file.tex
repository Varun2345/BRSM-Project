...
Normality tests showed non-normal distributions, but sample sizes (78 to 88) were sufficient. Variance homogeneity was confirmed.

Participants in the NB group recognised significantly more targets ($M = 87.0\%$) than those in the AB group ($M = 83.8\%$), $t(164) = -2.715$, $p = .007$, confirming that disrupting natural event boundaries produces a mild but reliable reduction in visual recognition memory.

\textbf{H1a (Accuracy):} Since $t(164) = -2.715$ and $p = .007 < \alpha = .05$, we \textbf{reject the null hypothesis} ($H_0$: no difference in accuracy between conditions) and \textbf{support the alternative hypothesis} ($H_A$: NB participants show higher recognition accuracy than AB participants).

\subsubsection*{5.1.2\quad Response Time}

Response times were positively skewed (typical for RT data), and variance was similar across groups. Contrary to prediction, NB participants ($M = 5.75$ s) were not noticeably faster than AB participants ($M = 5.57$ s); the difference was not statistically significant, $t(164) = -0.761$, $p = .448$.

\textbf{H1b (Response Time):} Since $t(164) = -0.761$ and $p = .448 > \alpha = .05$, we \textbf{fail to reject the null hypothesis} ($H_0$: no difference in RT between conditions). There is insufficient evidence to support the alternative hypothesis ($H_A$: NB participants respond faster than AB participants).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{plots/h1_accuracy.png}
    \caption*{Figure 5.1a --- H1: Mean recognition accuracy by condition (NB vs.\ AB).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{plots/h1_rt.png}
    \caption*{Figure 5.1b --- H1: Mean response time by condition (NB vs.\ AB).}
\end{figure}
...
\begin{table}[H]
    \centering\small
    \caption*{Table 5.4.\; Mean confidence ratings by frame type and condition}
    \begin{tabular}{@{} l c c c @{}}
        \toprule
        Group & BB Conf. & EM Conf. & Diff. \\
        \midrule
        AB    & 4.02     & 4.12     & $-$0.10 \\
        NB    & 4.19     & 4.21     & $-$0.02 \\
        \bottomrule
...
Contrary to prediction, the AB group showed significantly \textbf{lower} confidence for BB frames ($M = 4.02$) than EM frames ($M = 4.12$), $t(77) = -3.266$, $p = .002$. In the NB group there was no significant difference between frame types, $t(87) = -0.680$, $p = .498$.

\textbf{H5:} For the AB group, since $t(77) = -3.266$ and $p = .002 < \alpha = .05$, we \textbf{reject the null hypothesis} ($H_0$: confidence is equal for BB and EM trials) --- however, the direction is \textbf{opposite} to the alternative hypothesis ($H_A$: BB confidence $>$ EM confidence); BB confidence was in fact \textit{lower}. For the NB group, since $t(87) = -0.680$ and $p = .498 > \alpha = .05$, the alternative hypothesis is \textbf{not supported}.
...
\begin{itemize}
    \item \textbf{Accuracy (H1a--H3):} While AB participants were slightly less accurate, these differences were not statistically strong enough to pass more rigorous testing. The drop in accuracy was similar for both the middle and end of the clips, meaning the disruption wasn't limited to just the frames near the cut.
